...
public:
	...
	{
		double max=1e2, min=0, Iv_eq, phi_eq, rho_eq, P_eq, alt_gct = this->g*cos(alt_theta*pi/180.0), chi_eq, fb;
		const double pi = 3.14159265358979323846264338327950288;
		while (max-min > 1e-14)
		{
			phi_eq = this->pp.phim/(1+sqrt((max+min)/2.0));
			...
			fb = this->MuIv(0.5*(max+min))-tan(alt_theta*pi/180.0)/P_eq+this->tau0/((rho_eq-this->pp.rhof)*alt_gct*h);
			((fb>0)?max:min)=0.5*(max+min);
		}
		Iv_eq = 0.5*(max+min);
		phi = this->pp.phim/(1+sqrt(Iv_eq));
		rho_eq = phi*this->pp.rhog + (1-phi)*this->pp.rhof;
