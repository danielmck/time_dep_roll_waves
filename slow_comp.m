...
%         flux = max(final_pe./(rho.*g.*cosd(theta).*final_h));
...
%             flux = flux + (final_hu(k)+final_hu(k-1))/2*(final_grid(k)-final_grid(k-1))/wave_len;
%         end
        flux_vals(i,j) = flux;
    end
    
end
save("four_eqn_var_rho/flux_decrease_5deg_frac_static_fine.txt","flux_vals","-ascii")
