...
public:
	...
	{
		double max=1e-2, min=0, phi_eq, rho_eq, P_eq, Iv_eq;
		while (max-min > 1e-14)
		{
	...
	void SteadyUniformUTheta(double alt_theta, double h, double &u, double &phi)
	{
		double max=1e2, min=0, Iv_eq, phi_eq, rho_eq, P_eq, alt_gct = this->g*cos(this->alt_theta*pi/180.0);
		while (max-min > 1e-14)
		{
			phi_eq = this->pp.phim/(1+sqrt((max+min)/2.0));
...
public:
	...
	void SourceTerms(double dt, double *stvect, double *u, const double *extras, const double *dedt)
	{
		...
		double S_lim = 5, phi_rcp = 0.63;
		double phi_alt = pp.phim + (absu/h<S_lim)*(phi_rcp-pp.phim)*(S_lim-absu/h)/S_lim;
        double c1 = -2/(beta*h);
        double c2 = (rhoBulk-this->pp.rhof)*this->gcostheta*h;
        double c3 = 3*absu*phi;
        double c4 = 3/beta/h*phi;
        double c5 = 3*absu*phi_alt;
        double c6 = 3/beta/h*phi_alt;
        double c7 = 3*absu*pp.etaf/h;
        double c8 = rhoBulk*this->gcostheta*h;
        
        double coeff1 = c1-c4+c6;
        double coeff2 = c1*sqrt(c7);
        double coeff3 = -c1*c2+c3-c5+c8*(c4-c6);
        double coeff4 = c3*sqrt(c7)-c1*sqrt(c7)*c2;
		std::vector<std::complex<double>> sol;
		cubicFormula(coeff1,coeff2,coeff3,coeff4,sol);
		double root;
		// if (std::abs(sol[0].imag()) < 1e-12)
		// {
		// 	root = sol[0].real();
		// }
		// else
		// {
		// 	root = -1;
		// 	std::cout << "First root is complex, aah";
		// }
		root = sol[0].real();
		double ppval = root*root;
